...
class OmgCommand extends Command
{
    ...
    private function validateAPI()
    {
        ...
        $this->api->validate();
        $this->console->region('Specs validation warnings/errors', function () {
            $this->console->dumpArray($this->api->getErrors());
        });
    }
    ...
    {
        Folder::flush(Path::join(Config::$destination, 'components'));
        Folder::flush(Path::join(Config::$destination, 'lib'));
        Folder::flush(Path::join(Config::$destination, 'path'));
        $ns = Omg::getLibPath();
