...
class Omg
{
    private static array $generatedItems = [];
    ...
    public static function getGenerator($bodySchema, string $namespace, string $schemaLocation, string $type = null)
    {
        ...
        if ($bodySchema and $bodySchema instanceof Reference) {
            ...
            */
            ...
            if (!($generator instanceof SchemaBlankGenerator)) {
    ...
    public static function validateSchema(Schema $schema): void
    {
    ...
    public static function getClassnameSuffix($ref): string
    {
        ...
        else if (preg_match('/^\#\/components\/schemas\/\w+$/m', $ref)) {
            return 'Schema';
        }
        else if (preg_match('/^\#\/components\/requestBodies\/\w+$/m', $ref)) {
            return 'RequestBody';
        }
    ...
    public static function getReferenceClassPath($ref): string
    {
        ...
        else if (self::isComponentSchema($ref)) {
            $type = 'schema';
        }
        else if (self::isComponentRequestBody($ref)) {
            $type = 'requestBodies';
        }
        else {
    ...
    public static function notImplementedYet(array $extraData = [])
    {
        self::error('this part of the code is not implemented yet', $extraData);
    }
    ...
    public static function getType($resource): string
    {
        if ($resource instanceof Reference) {
        ...
        else if ($resource instanceof ParametersSpec) {
            ...
            self::error('unknown type', ['schema' => $resource]);
